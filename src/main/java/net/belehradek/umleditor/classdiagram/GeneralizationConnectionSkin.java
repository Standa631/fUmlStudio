...
public class GeneralizationConnectionSkin extends DefaultConnectionSkin {

	private static final String STYLE_CLASS = "generalization-connection";
	
	protected Arrow arrow = new Arrow();
	protected Group root;

	public GeneralizationConnectionSkin(GConnection connection) {
		super(connection);
		arrow = new Arrow();
	...
	public void draw(List<Point2D> points, Map<GConnection, List<Point2D>> allPoints) {
		...
		if (points.size() >= 2) {
			...
			arrow.setHeadWidth(20);
			arrow.setHeadLength(30);
